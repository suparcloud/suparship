...
func TestPublishAppFiles_BoundEnvsWriteFiles(t *testing.T) {
	...
	for _, envName := range []string{"staging", "prod"} {
		appYAML := filepath.Join(dir, "envs", envName, "demo", "hello", "app.yaml")
...
func TestPublishAppFiles_UnboundEnvSkipped(t *testing.T) {
	...
	stagingApp := filepath.Join(dir, "envs", "staging", "demo", "hello", "app.yaml")
	...
	prodApp := filepath.Join(dir, "envs", "prod", "demo", "hello", "app.yaml")
