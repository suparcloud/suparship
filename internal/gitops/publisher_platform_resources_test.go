...
func TestPublishAppFiles_WritesConfigMap(t *testing.T) {
	...
	for _, envName := range []string{"staging", "prod"} {
		...
		cmPath := filepath.Join(dir, envName, "demo", "nginx", "env-configmap.yaml")
...
func TestPublishAppFiles_NoExternalSecretWithoutStoreName(t *testing.T) {
	...
	esPath := filepath.Join(dir, "staging", "demo", "nginx", "external-secret.yaml")
...
func TestPublishAppFiles_CustomNamingPatterns(t *testing.T) {
	...
	cmPath := filepath.Join(dir, "prod", "billing", "api", "env-configmap.yaml")
	...
	esPath := filepath.Join(dir, "prod", "billing", "api", "external-secret.yaml")
