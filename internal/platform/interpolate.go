...
func (c Context) replacer() *strings.Replacer {
	...
	named := []string{
		...
		"platform.app", p.App,
		"platform.component", p.Component,
		"platform.env", p.Env,
		...
		"platform.appRoutingName", p.AppRoutingName,
...
func PlatformTokens() []TokenInfo {
	return []TokenInfo{
		...
		{Token: "((platform.component))", Label: "Component", Group: "Identity", Description: "the component's name (composed apps); the sole component of a single-component app"},
		...
		{Token: "((platform.appRoutingName))", Label: "App routing name", Group: "Routing", Description: "the app's hostname label — \"app\" (preview: \"app-pr-42\"; while routed to a preview the env is \"app-origin\"); compose with a base domain"},
