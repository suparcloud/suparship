...
func TestHandleAppValuesPreview_OmitsCanonicalBase(t *testing.T) {
	// The preview must NOT include any injected app/platform/suparship block —
	// only chart defaults + overlays, matching what the publisher publishes.
	store := newMemAppStore()
	store.addApp(&domain.App{
		Name:        "byo",
		ProjectName: "demo",
	...
	for _, k := range []string{"app", "platform", "suparship", "routing"} {
		if _, present := resp.Values[k]; present {
			t.Errorf("preview must omit injected key %q: %v", k, resp.Values)
		}
	}
}
