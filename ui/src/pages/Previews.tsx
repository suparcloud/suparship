        ...
        <div className="space-y-3">
          {previews.map((p) => (
            <PreviewCard
              key={p.name}
              preview={p}
              isConfirmingDelete={deleting === p.name}
              onRequestDelete={() => setDeleting(p.name)}
              onCancelDelete={() => setDeleting(null)}
              onConfirmDelete={() => handleDelete(p.name)}
            />
          ))}
        </div>
      )}
    ...
    <div className="flex items-start justify-between">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {count === 0
            ? "Ephemeral preview environments for your services."
...
function PreviewCard({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white transition-shadow hover:shadow-sm">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex flex-wrap items-center gap-x-3 text-xs text-gray-400">
                ...
                <Link
                  to={`/projects/${preview.project}/services/${preview.service}`}
                  className="hover:text-gray-600"
                >
                  {preview.service}
                </Link>
      ...
      {/* URL bar (mobile-friendly) */}
      {preview.url && (
        <div className="border-t border-gray-100 bg-gray-50 px-5 py-2 sm:hidden">
          <a
            href={preview.url}
            target="_blank"
            rel="noopener noreferrer"
  ...
  return (
    <div className="rounded-xl border border-dashed border-gray-300 bg-white px-6 py-16 text-center">
      ...
      <p className="mx-auto mt-1 max-w-sm text-sm text-gray-500">
        Create a preview from any service detail page to test changes in an
        isolated environment before promoting.
      </p>
    </div>
  );
}
